...
function Playlists() {
    ...
    const deletePlaylist = async (playlist_id, playlist_name) => {
        ...
        const response = await fetch(
            `${API_ENDPOINT}/api/playlists/${playlist_id}`,
            {
                method: "DELETE",
            }
        );
    ...
    const confirmDelete = (playlist_id, playlist_name) => {
        if (
            window.confirm(`Are you sure you want to delete ${playlist_name}?`)
        ) {
            deletePlaylist(playlist_id, playlist_name);
        }
    };

    // Navigate to the playlist edit page, sending state props to the edit page/component
    ...
    // Refreshes the playlists table when data changes
    useEffect(() => {
        loadPlaylists();
    }, []);
    return (
        <div>
            ...
            <form className="form-create">
                ...
                />
                <label htmlFor="is-private" className="form-label">
                    Private Playlist?
                </label>
                <select
                    name="isPrivate"
